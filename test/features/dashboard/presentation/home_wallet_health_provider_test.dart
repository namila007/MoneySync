...
/// LKR 0.00.
void main() {
  test('home health reads amount + currency from the linked mutation payload',
      () async {
    final db = AppDatabase.inMemoryForTesting();

    final eventId = await db.into(db.smsEvents).insert(
    ...
    await db.into(db.transactionCandidates).insert(
    ...
    await db.into(db.walletMutations).insert(
      WalletMutationsCompanion.insert(
        ...
        payload: '{"accountId":"account-1","amountMinor":-123456,'
            '"currencyCode":"LKR"}',
    ...
    await db.into(db.walletRecordLinks).insert(
  ...
  test('home health degrades to LKR 0.00 when the payload has no amount',
      () async {
    final db = AppDatabase.inMemoryForTesting();
