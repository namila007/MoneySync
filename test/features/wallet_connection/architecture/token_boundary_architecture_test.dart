...
void main() {
  group('WalletToken boundary', () {
    ...
    test('toPersistenceString exposes raw token — known WP4 gap', () {
      ...
      expect(
        ...
        reason:
            'toPersistenceString returns raw token value — '
    ...
    test('WalletToken.parse rejects empty tokens', () {
      expect(() => WalletToken.parse(''), throwsA(isA<FormatException>()));
    });
  ...
  group('KeystoreWalletSecretStore boundary', () {
    test(
      'token hex encoding uses utf8.encode in KeystoreWalletSecretStore',
      () {
    ...
    test('WalletSecretStore.useSecret scopes token access to a single '
      ...
      expect(_productionCallSitesReturnTypeIsNotWalletToken(), isTrue);
    });

    test('ProductionWalletConnectionActions resolves through the single '
