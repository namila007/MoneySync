...
void main() {
  Future<(AppDatabase, ProviderContainer)> build({
    ...
    await db.into(db.smsEvents).insert(
  ...
  test('privacy epoch gate reads real event vs settings epoch (M5.14 gap 2)',
      () async {
    // Event captured at epoch 0, settings advanced to 1 -> gate 1 must block.
    final (_, container) = await build(eventPrivacyEpoch: 0, settingsPrivacyEpoch: 1);
    ...
    final evaluation = container.read(reviewTransactionControllerProvider(1)).evaluation!;
    expect(evaluation.firstBlockedGateIndex, 0);
    expect(evaluation.firstBlockReason, contains('privacy epoch'));
  });

  test('consent gate reads disclosure + onboarding from app settings (gap 2)',
      () async {
  ...
  test('capability gate reads the capability ledger (empty -> blocks) (gap 2)',
      () async {
    final (_, container) = await build(
      eventPrivacyEpoch: 0,
      settingsPrivacyEpoch: 0,
  ...
  test(
    'submit with all gates passing writes one mutation with the serialized '
    'payload (M5.14 gap 2 + gap 3)',
    () async {
      ...
      final result = container.read(reviewTransactionControllerProvider(1)).result;
